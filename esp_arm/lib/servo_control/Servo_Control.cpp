...
{
    IDLE,
    GOTO_SLOT,  // move arm to pick position  (IK-solved)
    GRIP_CLOSE, // close gripper on box
    GOTO_DROP,  // move arm to drop position  (IK-solved)
    GRIP_OPEN,  // open gripper, release box
    GOTO_FOLD,  // move arm to compact fold position (IK-solved)
...
static ArmPose dropFold = {ARM_HOME_J1, ARM_HOME_J2, ARM_HOME_J3};
...
// Resolves XYZ → IK angles via Place_*_Box(), which also pushes
// the DropSequenceCmd onto dropSeqMailbox.
...
void Servo_Control_Task(void *arg)
{
    ...
    while (1)
    {
        ...
        if (dropPhase != DropPhase::IDLE)
        {
            ...
            case DropPhase::GOTO_SLOT:
                if (dropFirstEntry)
                {
                    ...
                    Serial.printf("[DROP] → PICK SLOT (IK) J1=%d J2=%d J3=%d\n",
            ...
            case DropPhase::GOTO_DROP:
                if (dropFirstEntry)
                {
                    ...
                    Serial.printf("[DROP] → DROP ZONE (IK) J1=%d J2=%d J3=%d\n",
            ...
            case DropPhase::GRIP_OPEN:
                ...
                {
                    dropPhase = DropPhase::GOTO_FOLD;
                    dropFirstEntry = true;
                }
                break;

            case DropPhase::GOTO_FOLD:
                if (dropFirstEntry)
                {
                    dropTimer = millis();
                    dropFirstEntry = false;
                    writeArmPose(dropFold);
                    Serial.printf("[DROP] → FOLD POSE (IK) J1=%d J2=%d J3=%d\n",
                                  dropFold.j1, dropFold.j2, dropFold.j3);
                }
                if (millis() - dropTimer >= DROP_TIME_GOTO_FOLD)
                {
        ...
        {
            // IK poses were already resolved by Place_*_Box() and stored in gIKCache
            if (!gIKCache.valid)
            {
                Serial.println("[SERVO] WARNING: IKPoseCache invalid — sequence skipped.");
                vTaskDelayUntil(&xLastWakeTime, xFreq);
                continue;
            }

            dropSlot = gIKCache.pick;
            dropTarget = gIKCache.drop;
            dropFold = gIKCache.fold;
            gIKCache.valid = false; // consume the cache

            Serial.printf("[DROP] Starting IK sequence for colour: %s\n", dsCmd.color);
