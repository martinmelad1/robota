...
void UART_Master_Init(void) {
  ...
  uart_set_pin(UART_CAM, CAM_TX, CAM_RX, UART_PIN_NO_CHANGE,
               UART_PIN_NO_CHANGE);
  uart_driver_install(UART_CAM, BUF_SIZE, 0, 0, NULL, 0);

  // Arm UART
  uart_param_config(UART_ARM, &config);
  uart_set_pin(UART_ARM, ARM_TX, ARM_RX, UART_PIN_NO_CHANGE,
               UART_PIN_NO_CHANGE);
...
void UART_Cam_Task(void *arg) {
  ...
  while (1) {
    int len =
        uart_read_bytes(UART_CAM, data, BUF_SIZE - 1, 20 / portTICK_PERIOD_MS);
    ...
    if (len > 0) {
      ...
      if (ipPtr != NULL) {
        ipPtr += 7; // Skip "CAM_IP:"
        // Copy IP, stop at newline, carriage return, space, or non-printable
        int j = 0;
        while (ipPtr[j] && ipPtr[j] != '\n' && ipPtr[j] != '\r' && ipPtr[j] != ' ' && j < 19) {
...
void UART_Arm_Task(void *arg) {
  ...
  while (1) {
    ...
    if (xQueueReceive(armMailbox, &motion_cmd, 0) == pdTRUE) {
      ...
      if (motion_cmd.joint_id ==
          6) { // Virtual joint for instant gripper commands
    ...
    int len =
        uart_read_bytes(UART_ARM, data, BUF_SIZE - 1, 20 / portTICK_PERIOD_MS);
    ...
    if (len > 0) {
      ...
      if (hasPrintable) {
        ...
        // Parse DROP_DONE:<color> from ARM
        char *dropDonePtr = strstr((char *)data, "DROP_DONE:");
        if (dropDonePtr != NULL) {
          StringMessage sm;
          strncpy(sm.data, dropDonePtr, sizeof(sm.data) - 1);
          sm.data[sizeof(sm.data) - 1] = '\0';
