...
void QR_Reader_Init() {
    ...
    QRCodeReaderSetupErr err = qrReader.setup();
    if (err != SETUP_OK) {
        Serial.printf("\n\n!!! CRITICAL ERROR !!!\nCamera failed to initialize! Error Code: %d\n", err);
        Serial.println("Check your camera wiring, make sure the ribbon cable is seated properly, and ensure sufficient 5V power!");
    } else {
        Serial.println("Camera hardware initialized perfectly.");
    }
    ...
    if (s) {
        ...
        Serial.println("Sensor settings applied successfully.");
    } else {
        Serial.println("WARNING: Could not get camera sensor object! The camera might be disconnected.");
    }

    Serial.println("QR Reader Background Task Started (Core 1).");
}
